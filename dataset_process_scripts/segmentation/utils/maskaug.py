...
class MaskAugAnnotator:
    ...
    def apply_seg_mask(self, mask_data, frames, mask_color, mask_cfg=None):
        ...
        out_frames = frames
        out_masks = [np.zeros(frames[i].shape[:-1], dtype=np.uint8) for i in range(len(frames))]
        for k, masks in mask_data.items():
            num_frames = min(len(masks), len(frames))
            for i in range(num_frames):
                sub_mask = masks[i]
                if mask_cfg is not None:
                    sub_mask = self.forward(sub_mask, mask_cfg)
                    out_masks[i] += sub_mask
                bool_mask = sub_mask > 0
                out_frames[i][bool_mask] = mask_color
