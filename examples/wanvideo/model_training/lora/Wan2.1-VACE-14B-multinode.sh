#!/bin/bash
# ============================================================================
# Wan2.1-VACE-14B LoRA Multi-Node Training Script
# ============================================================================
#
# 多机分布式 LoRA 训练启动脚本。需在每台机器上分别运行，通过 --node_rank 区分节点。
#
# ========================= 必填参数 =========================================
...
#   --lora_target_modules  LoRA 目标模块 (默认: "q,k,v,o,ffn.0,ffn.2")
...
# ========================= 与 full 训练的区别 ================================
#
# - LoRA 仅训练低秩适配器，显存占用更少，无需 CPU offload
# - 默认 lora_rank=32, 目标模块: q,k,v,o,ffn.0,ffn.2
# - 学习率默认 1e-4 (full 训练为 5e-5)
#
# ============================================================================
...
# ---- Training defaults (same as single-node LoRA script) -------------------
...
WANDB_PROJECT="VACE"
EXPERIMENT_NAME="Wan2.1-VACE-14B-LoRA-MultiNode"
WANDB_MODE="online"
WANDB_LOG_STEPS=500
WANDB_RUN_ID=""
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "                 --lora_rank --lora_target_modules"
...
# ---- Print configuration ---------------------------------------------------
echo "============================================"
echo " Multi-Node LoRA Training Configuration"
echo "============================================"
